...
class Game {
  ...
  render() {
    ...
    this.canvasCtx.fillStyle = this.canvasCtx.createPattern(this.player.room.canvasList[this.player.orientation], "no-repeat");
    this.canvasCtx.save();
    const playerPosition = this.player.room.translateFrom(this.player.orientation, this.player);
    this.canvasCtx.translate(
      Math.floor(this.screenCenter.x - playerPosition.x - 24),
      Math.floor(this.screenCenter.y - playerPosition.y - 24)
    );
