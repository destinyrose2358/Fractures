...
class Game {
  ...
  renderRoom(room, orientation, offset) {
    this.canvasCtx.fillStyle = this.canvasCtx.createPattern(
    ...
    this.canvasCtx.translate(
      Math.floor(this.screenCenter.x - playerPosition.x - 24 + offset.x),
      Math.floor(this.screenCenter.y - playerPosition.y - 24 + offset.y)
    );
        ...
        room.room.doors.forEach(door => {
          ...
          switch (translatedDoor.orientation) {
            case 0:
              ...
              newY = - (newRoomDimensions.height + 1) * 16;
              break;
            case 1:
              newX = (oldRoomDimensions.width + 1) * 16;
              newY = (translatedDoor.tile - translatedThroughDoor.tile) * 16;
              break;
            case 2:
              newX = (translatedDoor.tile - translatedThroughDoor.tile) * 16;
              newY = (oldRoomDimensions.height + 1) * 16;
              break;
            case 3:
              newX = - (newRoomDimensions.width + 1) * 16;
              newY = (translatedDoor.tile - translatedThroughDoor.tile) * 16;
              break;
          }
          ...
          if (!rooms.includes(newRoomObject)) {
